...
public class DebugWsController {
    ...
    public Result<Boolean> pushAlert(@RequestBody(required = false) Map<String, Object> body) {
        try {
            ...
            payload.put("title", body.getOrDefault("title", "测试告警 via ws-debug"));
            payload.put("message", body.getOrDefault("message", "这是一条来自 /ws-alert/debug 的测试告警"));
            payload.put("severity", body.getOrDefault("severity", "low"));
